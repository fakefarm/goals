...
/*
 * The `app` var here was passed in from the `require('./config/routes')(app)`
 * of `index.js` file. Many times you will return a value from this, e.g.
 * `var express = require('express')` but in this case, we are just adding to
 * our `app` object & not assigning this module to a variable or anything.
 *
 * I would probably move the callback functions for these routes (our logic)
 * along with the above functions/variables into controller(s), but that's more
 * of a preference and is not required or anything. You are allowed to choose
 * your own structure with Node. For simple apps, this setup is fine. You could
 * always refactor later and move the logic into their own file(s).
 *
 */
